...
def pick_adj(non_empty_list, list, weights_sublist, length, operation): ######changed chosen_length to length

  #pick a sublist
  sub_list1=[]
  count=0

  while (len(sub_list1)==0 and count==0) or (len(sub_list1)==0 and (operation=="DCJ" or operation=="Deletion")):
  ...
  if operation=="DCJ":
     ...
     while(sub_pos1==sub_pos2 and adj1==adj2)or adj2==-5:
           ...
           if 0 in sub_list2:
              ...
              print("adj2: " + str(adj2))
              while adj2==-5 or adj2==len(sub_list2)-1 or (adj1==adj2 and sub_pos1==sub_pos2):
...
def main():
       ...
       for q in range(int(avg)):
                ...
                for p in range(0,int(num_steps)):
                      ...
                      #num_empty_list:keeps track of how many empty lists(circular) there is 
                      num_empty_list=0
                      non_empty_list=0
                      for m in list:
                         if not m:
                      ...
                      #creating the weighst each sub_list
                      ...
                      for num in all_gene:
                         if num != 0:
                            ...
                            num_zero+=1
                      #when a circular chromosome is empty, it still gets 1 weights so that new genes can be inserted
                      #adding the extra 1s for the empty circular lists to count for it to be added to N
                      count=count+ num_empty_list
                      
                      #N:total number of adjecencies in the genome
                      N=count+ num_zero/2
                      ...
                      for m in list:
                           ...
                           for j in m:
                               if j != 0:
                                   ...
                                   sub_zero+=1
                           #empty circular lists getting 1/N as weight
                           if not m:
                              sub_count+=1
                      ...
                      if operation=='DCJ':
                               ...
                               adj1, adj2, sub_pos1, sub_pos2, final_length=pick_adj(non_empty_list, list, weights_sublist, chosen_length, operation)
